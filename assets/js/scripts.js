...
var arrowUp = document.getElementById("uparrow");
var arrowDown = document.getElementById("downarrow");
var arrows = document.getElementById("arrows");
var arrowsAlt = document.getElementById("arrows-alt");
var pos = 0;
var posY = 0;
const len = carrousel != null ? carrousel.children.length : 0;

var cards = document.querySelectorAll('.feed__item');
...
function dimensions() {
    if(window.innerWidth <= 768) {
        ...
        if(arrowsAlt) arrowsAlt.style.display = `flex`;
        if(carrousel){
            carrousel.style.width = `100%`;
            carrousel.style.minWidth = `100%`;
            carrousel.style.height = `${13.2 * len}rem`;
            carrousel.style.transform = `translateX(0%)`;
        }
        if(cards){
            cards.forEach(function(item) {
                item.style.width = `96%`;
                item.style.minWidth = `96%`;
            })
        }
    }
    else {
        var len_carrousel = 100 / 3 * len;
        var len_card = 100 / len;
        if(arrows) arrows.style.display = `flex`;
        if(arrowsAlt) arrowsAlt.style.display = `none`;
        if(carrousel){
            carrousel.style.width = `${Math.max(len_carrousel, 100)}%`;
            carrousel.style.minWidth = `${Math.max(len_carrousel, 100)}%`;
            carrousel.style.height = `fit-content`;
            carrousel.style.transform = `translateY(0%)`;
        }
        if(cards){
            cards.forEach(function(item) {
                item.style.width = `${len_card}%`;
                item.style.minWidth = `${len_card}%`;
            })
        }
    }
}
...
if(arrowLeft){
    arrowLeft.addEventListener('click', () => {
        ...
        if(pos < 0) pos = pos + 1;
        if(carrousel) carrousel.style.transform = `translateX(${pos*len_card}%)`
...
if(arrowUp){
    arrowUp.addEventListener('click', () => {
        ...
        if(carrousel) carrousel.style.transform = `translateY(${posY*len_card}%)`
...
$(function () {
    $('#form-contacto').submit(function (e) {
        e.preventDefault();
        var $frm = $("#form-contacto");
        console.log("llamando formulario....");
